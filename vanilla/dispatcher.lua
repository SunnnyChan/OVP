-- vanilla
local Controller = require 'vanilla.controller'
local Request = require 'vanilla.request'
local Router = require 'vanilla.router'
local Response = require 'vanilla.response'
local View = require 'vanilla.views.rtpl'
...
local function call_controller(Dispatcher, matched_controller, controller_name, action_name)
    ...
    local ok, body
    -- 两种调用action方式
    if type(matched_controller[action_name]) ==  "function" then
        ok, body = matched_controller[action_name](matched_controller)
    else 
        ok, body = matched_controller:default()
    end
    -- action not exist
    if ok == false then
        Dispatcher:errResponse({ code = 102, msg = {ActionErr = action_name}})
    end
    if body ~= nil then
        return body
    else
...
function Dispatcher:lpcall( ... )
    ...
    if ok then
        ...
        ngx.log(ngx.WARN, rs_or_error)
        self:errResponse({ code = 103, msg = {Rs_Error = 'controller may not exist'}})
    end
end
