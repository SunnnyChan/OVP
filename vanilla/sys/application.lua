-- dep
local ansicolors = require 'vanilla.libs.ansicolors'

-- vanilla
local va_conf = require 'vanilla.sys.config'
local utils = require 'vanilla.libs.utils'
...
local dao = [[
-- local TableDao = require('vanilla.model.dao'):new()
local TableDao = {}
...
local service = [[
-- local UserService = require('vanilla.model.service'):new()
...
Appconf.app={}
Appconf.app.root='{{VANILLA_PATH}}/app/' .. Appconf.name
...
function VaApplication.new(name)
    ...
    local app_conf = string.gsub(application_conf, "{{APP_NAME}}", name)
    VaApplication.files['config/application.lua'] = string.gsub(app_conf, "{{VANILLA_PATH}}", VANILLA_PATH )
...
function VaApplication.create_files(parent)
    for file_path, file_content in pairs(VaApplication.files) do
        local full_file_path = VANILLA_PATH .. "/app/" .. parent .. "/" .. file_path
